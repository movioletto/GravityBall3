 ...
 * @author Movioletto
 */
public class SchermataIstruzioni1 implements Screen
{
...
//	private Immagine istruzioni;
	...
	private Immagine istm1;
	private Immagine istm2;
	 ...
	 */
	public SchermataIstruzioni1(Game game)
	{
		this.dispose=false;
		this.game=game;
...
//		this.istruzioni=new Immagine(new Vec2(Cost.SCHERMOWIDTH/2, Cost.SCHERMOHEIGHT/2), Tipo.ISTRUZIONI1);
		...
		Testo[] test={
			...
			new Testo(Tipo.TESTO70, "dai muri e colleziona", new Vec2(Cost.SCHERMOWIDTH/2+15, 520)),
			new Testo(Tipo.TESTO70, "piu' soldi possibili!", new Vec2(Cost.SCHERMOWIDTH/2+30, 480)),
		};
		
		testi=test;
		if(Cost.touch && Cost.accel)
		{
			this.istm1=new Immagine(new Vec2(Cost.SCHERMOWIDTH/4, Cost.SCHERMOHEIGHT/4+50), Tipo.MIXSD);
			this.istm2=new Immagine(new Vec2((Cost.SCHERMOWIDTH/4)*3, Cost.SCHERMOHEIGHT/4+50), Tipo.MIXSG);
		}
		else if(!Cost.touch && Cost.accel)
		{
			this.ist1=new ImmagineAnimata(new Vec2(Cost.SCHERMOWIDTH/4, Cost.SCHERMOHEIGHT/4+50), 2);
			this.ist2=new ImmagineAnimata(new Vec2((Cost.SCHERMOWIDTH/4)*3, Cost.SCHERMOHEIGHT/4+50), 3);
		}
		else if(Cost.touch && !Cost.accel)
		{
			this.ist1=new ImmagineAnimata(new Vec2(Cost.SCHERMOWIDTH/4, Cost.SCHERMOHEIGHT/4+50), 0);
			this.ist2=new ImmagineAnimata(new Vec2((Cost.SCHERMOWIDTH/4)*3, Cost.SCHERMOHEIGHT/4+50), 1);
		}
		
		this.bottoni=new Bottone(Tipo.TESTO70, "gioca!", new Vec2((Cost.SCHERMOWIDTH/4.0f)*3, 75.0f), this.mondo, 0, game);
...
//		this.istruzioni.paint(sprite);
		...
		if(Cost.touch && Cost.accel)
		{
			this.istm1.paint(sprite);
			this.istm2.paint(sprite);
		}
		else
		{
